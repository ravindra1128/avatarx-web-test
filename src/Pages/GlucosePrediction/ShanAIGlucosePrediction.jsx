...
export default function NewGlucosePredicttion() {
  ...
  const ppgSignalDataRef = useRef(null);
  ...
  const buildPpgPayload = useCallback((ppgData, formValues = null) => {
    console.log("ppgData", ppgData, shenaiResultsRef.current, formValues);
    ...
    // Use form values if provided, otherwise fall back to shenai results
    const genderValue = formValues?.gender 
      ? (formValues.gender.toLowerCase() === "male" ? 1 : formValues.gender.toLowerCase() === "female" ? 0 : 1)
      : (shenaiResults.gender ?? 1);
    
    const ageValue = formValues?.age 
      ? parseInt(formValues.age) 
      : (shenaiResults.age_years ?? undefined);
    
    const heightValue = formValues?.height 
      ? parseFloat(formValues.height) 
      : (shenaiResults.height_cm ?? undefined);
    
    const weightValue = formValues?.weight 
      ? parseFloat(formValues.weight) 
      : (shenaiResults.weight_kg ?? undefined);
    
    const systolicBP = formValues?.systolicBP 
      ? parseFloat(formValues.systolicBP) 
      : (shenaiResults.systolic_blood_pressure_mmhg ?? undefined);
    
    const diastolicBP = formValues?.diastolicBP 
      ? parseFloat(formValues.diastolicBP) 
      : (shenaiResults.diastolic_blood_pressure_mmhg ?? undefined);
    
    const glucoseValue = formValues?.glucose 
      ? parseFloat(formValues.glucose) 
      : undefined;
    ...
    return {
      ...
      systolic_bp_device: formValues.systolicBP,
      diastolic_bp_device: formValues.diastolicBP,
      hrv_lnrmssd_ms: shenaiResults.hrv_lnrmssd_ms ?? undefined,
      hrv_sdnn_ms: shenaiResults.hrv_sdnn_ms ?? undefined,
      breathing_rate_bpm: shenaiResults.breathing_rate_bpm ?? undefined,
      Pulse_Area: pulseArea,
      "index(integer)": 1,
      "Gender(1 for Male, 0 for Female)": genderValue,
      "Height(cm)": heightValue,
      "Weight(kg)": weightValue,
      Age: ageValue,
      "Age Range[1,2,3,4,5]": shenaiResults.age_range_bucket ?? undefined,
      "Glucose(mg/dL)": glucoseValue,
      Patient_Id: 1
    };
  }, []);
  ...
  const sendPpgDataToServer = useCallback(
    async (signalData, formValues = null) => {
      if (!signalData || !shenaiResultsRef.current) return null;
      const payload = buildPpgPayload(signalData.shenaiPpg, formValues);
  ...
  const handleFormSubmit = useCallback(async () => {
    ...
    try {
      // Get the stored PPG signal data
      const signalData = ppgSignalDataRef.current;
      if (!signalData) {
        logWarn("No PPG signal data available");
        alert("Error: No measurement data available. Please try again.");
        setIsSubmitting(false);
        return;
      }
      
      // Call glucose prediction API with form data
      const glucoseData = await sendPpgDataToServer(signalData, formData);
      ...
      // Close form and navigate to results
      setShowFormModal(false);
      
      // Ensure SDK is not showing results screen before navigation
      if (shenai && typeof shenai.setScreen === "function") {
        // Keep on measurement screen to avoid showing SDK results
        shenai.setScreen(shenai.Screen.MEASUREMENT);
      }
      
      // Keep loader visible during navigation
      navigate("/data-capture-result", {
        state: {
          shenaiResults: shenaiResultsRef.current,
          glucoseResults: glucoseData || glucoseResultsRef.current,
          ppgSignal: signalData?.shenaiPpg || [],
        },
        replace: true,
      });
      
      // Note: isSubmitting will remain true until component unmounts or navigation completes
      // The loader will be visible during the entire process
    } catch (error) {
      logCritical(`Error submitting form and calling glucose API: ${error.message}`, { phase: 'form_submission', error: error.message, errorStack: error.stack });
      alert("Failed to submit. Please try again.");
      setIsSubmitting(false);
    }
  }, [formData, sendPpgDataToServer, navigate]);
  ...
  useEffect(() => {
    ...
    async function pollMeasurement(sdk) {
      ...
      if (measurementState === sdk.MeasurementState.FINISHED) {
        // finished = true;
        // Don't show SDK results screen - we'll show our custom form and results page
        ...
        //   }
          if (!finished) {
            ...
            // Store PPG signal data for later use
            ppgSignalDataRef.current = ppgSignalData;
            
            // Hide SDK UI before showing form modal
            if (shenai && typeof shenai.setScreen === "function") {
              // Keep on measurement screen or hide UI - don't show SDK results
              shenai.setScreen(shenai.Screen.MEASUREMENT);
            }
            
            // Show form modal instead of immediately calling API
  ...
  }, [shenai, initDone, authData, userLanguage, extractShenaiPpgData]);
    ...
    >
      ...
      >
        ...
        <Dialog open={showFormModal} onOpenChange={(open) => !open && setShowFormModal(false)}>
          <DialogContent className="max-w-sm max-h-[85vh] overflow-y-auto bg-white p-4">
            <DialogHeader className="">
              <DialogTitle className="text-base">Patient Information</DialogTitle>
            </DialogHeader>
            
            <div className="grid gap-2.5 py-2">
              <div className="grid gap-1.5">
                <Label htmlFor="age" className="text-xs text-left">Age</Label>
                <Input
                  id="age"
                  type="number"
                  placeholder="Age"
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="gender" className="text-xs text-left">Gender</Label>
                <Select
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="glucose" className="text-xs text-left">Glucose (mg/dL)</Label>
                <Input
                  id="glucose"
                  type="number"
                  placeholder="Glucose"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="grid gap-1.5">
                  <Label htmlFor="systolicBP" className="text-xs text-left">Systolic BP</Label>
                  <Input
                    id="systolicBP"
                    type="number"
                ...
                <div className="grid gap-1.5">
                  <Label htmlFor="diastolicBP" className="text-xs text-left">Diastolic BP</Label>
                  <Input
                    id="diastolicBP"
                    type="number"
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="height" className="text-xs text-left">Height (cm)</Label>
                <Input
                  id="height"
                  type="number"
                  placeholder="Height"
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="weight" className="text-xs text-left">Weight (kg)</Label>
                <Input
                  id="weight"
                  type="number"
                  placeholder="Weight"
            ...
            <DialogFooter className="">
              <Button
                ...
                size="sm"
                className="h-8 text-xs w-full bg-black text-white"
              >
