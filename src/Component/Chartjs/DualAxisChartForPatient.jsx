...
const DualAxisChartForPatient = ({ data = [], selectedVital = 'All' }) => {
  ...
  const dateTimeLabels = useMemo(() => {
    ...
    return data.map((item) => {
      ...
      const datePart = `${month} ${day}`.trim();
      const timePart = `${hour}:${minute}${dayPeriod ? ` ${dayPeriod}` : ''}`.trim();

      return [datePart || '—', timePart || '—']; // Chart.js renders array as two lines
    });
  }, [data]);
  ...
  const options = {
    ...
    scales: {
      ...
      hr: {
        type: 'linear',
        display: !!visibleAxes.hr,
        position: 'right',
        grid: { display: false },
        title: { display: !!visibleAxes.hr, text: 'Heart Rate (BPM)' },
      },
      br: {
        type: 'linear',
        display: !!visibleAxes.br,
        position: 'right',
        grid: { display: false },
        title: { display: !!visibleAxes.br, text: 'Breathing Rate (BPM)' },
      },
      hrv: {
        type: 'linear',
        display: !!visibleAxes.hrv,
        position: 'right',
        grid: { display: false },
        title: { display: !!visibleAxes.hrv, text: 'HRV (ms)' },
      },
      x: {
        grid: { display: true },
        ticks: {
...
export default DualAxisChartForPatient;